...
export const Navbar: React.FC<NavbarProps> = ({ onContactClick, onHomeClick, isHome = true }) => {
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-[160] transition-[padding,background-color] duration-700 px-6 py-6 ${scrolled ? 'pt-4' : 'pt-8'}`}>
        <div className={`max-w-[1600px] mx-auto transition-transform duration-700 relative ${scrolled ? 'scale-[0.99]' : 'scale-100'}`}>
          <div className={`relative flex items-center justify-between px-10 transition-[height,background-color,border-color,box-shadow] duration-700 rounded-full border ${scrolled
            ...
            <div className="hidden lg:flex items-center space-x-2">
              {NAV_LINKS.map((item) => (
                <button
                  ...
                  className={`px-8 py-2 text-[10px] font-black uppercase tracking-[0.45em] transition-all duration-500 relative group overflow-hidden ${scrolled ? 'text-slate-400 hover:text-slate-950' : 'text-slate-400 hover:text-slate-950'
                    }`}
                >
            ...
            <div className="flex items-center space-x-4">
              <button
                ...
                className={`hidden md:flex px-10 py-3.5 rounded-full text-[10px] font-black uppercase tracking-[0.45em] transition-[background-color,transform,box-shadow] duration-700 shadow-lg ${scrolled

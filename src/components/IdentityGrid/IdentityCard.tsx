...
export const IdentityCard: React.FC<IdentityCardProps> = ({ id, title, description, iconType }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`relative h-full spotlight-card p-10 rounded-[2.5rem] flex flex-col justify-between border transition-[background-color,border-color,shadow] duration-700 overflow-hidden ${isHovered
        ...
        <div className="relative z-10">
          <div className="flex justify-between items-start mb-12">
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-[background-color,color,transform,box-shadow] duration-500 ${isHovered ? 'bg-violet-600 text-white scale-110 shadow-lg shadow-violet-200' : 'bg-white text-slate-400 border border-slate-100 shadow-sm'
              }`}>
              <IconComponent />
            </div>
          ...
          <h4 className={`text-3xl font-black leading-[1.1] tracking-tighter mb-4 transition-[color,transform] duration-500 ${isHovered ? 'text-slate-950 translate-x-1' : 'text-slate-900'}`}>
            {title}
          </h4>

...
export const ServiceCard: React.FC<{ service: Service, index: number, onClick?: () => void }> = ({ service, index, onClick }) => {
  ...
  return (
    <motion.div
      ...
      className="group relative h-[380px] w-full rounded-3xl overflow-hidden bg-slate-100 shadow-sm cursor-pointer will-change-transform"
    ...
    >
      <motion.img
        ...
        src={service.img}
        alt={`Service overview: ${service.title}`}
        loading="lazy"
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-transparent via-slate-950/20 to-slate-950/60 opacity-60 group-hover:opacity-20 transition-opacity duration-700" />
      ...
      >
        <div className="relative z-20">
          <div className="flex items-center justify-between gap-4">
            <h4 className={`text-xl font-black text-white leading-tight tracking-tighter transition-[color,margin,transform] duration-300 ${isHovered ? 'mb-4' : 'mb-0'}`}>
              {service.title}
            </h4>
            <motion.div
